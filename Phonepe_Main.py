...
phn_path = r'D:\My files\Nagammai\Guvi\MDE86\code\Phonepelogo.jpg'
phn1_path = r'D:\My files\Nagammai\Guvi\MDE86\code\Phonepegif.jpg'

if os.path.exists(phn_path):
    phn_bytes = open(phn_path, 'rb').read()
    st.sidebar.image(phn_bytes, caption='Phonepe', width=150)

if os.path.exists(phn1_path):
    phn1_bytes = open(phn1_path, 'rb').read()
    st.sidebar.image(phn1_bytes, caption='Easy Transactions!!!', width=150)

# Display YouTube video
st.sidebar.video("https://www.youtube.com/watch?v=rCmXuJTEajA")

# Information text
st.write('''The Indian digital payments narrative has undeniably captivated global attention. From bustling urban centers to the far-flung corners of rural areas, a sweeping payments revolution is unfolding, propelled by the ubiquitous presence of mobile phones and data connectivity. PhonePe Pulse serves as your gateway into the intricacies of India's transaction landscape, offering compelling insights, profound trends, and comprehensive analysis curated from our data by the PhonePe team.''')
st.write('**(Note)**:-This data between **2018** to **2023** in **INDIA**')

# Selection option
option = st.selectbox('**Select your option**',('All India', 'State wise','Top Ten categories'))
# ===================================================       /      All India      /     ===================================================== #
if option == 'All India':
    # Select tab
    ...
    with tab2:
        ...
        fig_use = px.choropleth(
            df_use,
            geojson="https://gist.githubusercontent.com/jbrobst/56c13bbbf9d97d187fea01ca62ea5112/raw/e388c4cae20aa53cb5090210a42ebb9b765c0a36/india_states.geojson",
            featureidkey='properties.ST_NM',locations='State',color='User Count',color_continuous_scale='blues',title = 'User Analysis')
        ...
        df_in_us_tab_qry_rslt1_fig = px.bar(df_in_us_tab_qry_rslt1 , x = 'State', y ='User Count', color ='User Count', color_continuous_scale = 'greens', title = 'User Analysis Chart', height = 700,)
...
# ==============================================          /     State wise       /             ============================================== #
    ...
    with tab3:
        ...
        col1, col2,col3 = st.columns(3)
        with col1:
            st_tr_st = st.selectbox('**Select State**',('andaman_and_nicobar_islands', 'andhra_pradesh', 'arunachal_pradesh','assam', 'bihar', 
            'chandigarh', 'chhattisgarh','dadra_and_nagar_haveli_and_daman_and_diu', 'delhi', 'goa', 'gujarat', 'haryana', 'himachal_pradesh', 
            'jammu_and_kashmir', 'jharkhand', 'karnataka', 'kerala', 'ladakh', 'lakshadweep', 'madhya_pradesh','maharashtra', 'manipur', 
            'meghalaya', 'mizoram', 'nagaland','odisha', 'puducherry', 'punjab', 'rajasthan', 'sikkim', 'tamil_nadu', 'telangana', 
            'tripura', 'uttar_pradesh', 'uttarakhand', 'west_bengal'),key='st_tr_st')
        with col2:
        ...
        if not st_tr_tab_bar_qry_rslt:
            st.error("No data found for the specified conditions.")
        else:
            # Convert query result to DataFrame
            df_st_tr_tab_bar_qry_rslt = pd.DataFrame(st_tr_tab_bar_qry_rslt, columns=['Transaction_type', 'Transaction_amount'])
        ...
        # Check if the DataFrame is empty for the specified state
        if df_st_tr_co_qry_rslt1.empty:
            st.write(f"No data found for the state {st_us_st} and year {st_us_yr}.")
        else:
    ...
    with tab4:
        ...
        with col5:
            st_us_st = st.selectbox('**Select State**',('andaman_and_nicobar_islands', 'andhra_pradesh', 'arunachal_pradesh','assam', 'bihar', 
            'chandigarh', 'chhattisgarh','dadra_and_nagar_haveli_and_daman_and_diu', 'delhi', 'goa', 'gujarat', 'haryana', 'himachal_pradesh', 
            'jammu_and_kashmir', 'jharkhand', 'karnataka', 'kerala', 'ladakh', 'lakshadweep', 'madhya_pradesh','maharashtra', 'manipur', 
            'meghalaya', 'mizoram', 'nagaland','odisha', 'puducherry', 'punjab', 'rajasthan', 'sikkim', 'tamil_nadu', 'telangana', 
            'tripura', 'uttar_pradesh', 'uttarakhand', 'west_bengal'),key='st_us_st')
        with col6:
        ...
        cursor.execute(f"SELECT brands AS Brand, SUM(Count) AS Total_Transactions FROM aggregated_user WHERE State = '{st_us_st}' AND Year = '{st_us_yr}' GROUP BY brands")
        st_us_co_qry_rslt = cursor.fetchall()
        df_st_us_co_qry_rslt = pd.DataFrame(np.array(st_us_co_qry_rslt), columns=['Brand', 'Total_Transactions'])
        df_st_us_co_qry_rslt1 = df_st_us_co_qry_rslt.set_index(['Brand'])


        # -----   /   All India User Analysis Bar chart   /   ----- #
        # Check if the DataFrame is empty for the specified state
        df_st_us_co_qry_rslt1.reset_index(inplace=True)
        if df_st_us_co_qry_rslt1.empty:
            st.write(f"No data found for the state {st_us_st} and year {st_us_yr}.")
        else:
            ...
            df_st_us_co_qry_rslt1_fig = px.bar(df_st_us_co_qry_rslt1, x='Brand', y='Total_Transactions', color='Total_Transactions', color_continuous_scale='thermal', title='Brand Transaction Analysis', height=500)
            df_st_us_co_qry_rslt1_fig.update_layout(title_font=dict(size=33), title_font_color='#6739b7')
            st.plotly_chart(df_st_us_co_qry_rslt1_fig, use_container_width=True)
